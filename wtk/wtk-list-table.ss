...
(provide (all-defined-out)
         (all-from-out "wtk-list.ss"))

; table-column : symbol string string (a -> html) (a a -> boolean)
(define-struct table-column (id short-name long-name obj->html obj-<=) #:prefab)

; interleave : number (a -> html)
(define-struct interleave (every gen) #:prefab)

(define-struct table-ui-state (list-ui-state
                               objs-per-page current-page
                               highlight-pred? emphasis-pred?) #:prefab)
...
(define (with-table-ui embed/url 
  ...
  (define the-list-ui 
    (with-list-ui (table-ui-state-list-ui-state
                   (web-cell-ref table-ui-state-cell))
                  ; filters
                  filters
                  ; sorters
                  (map (lambda (a-column)
  ...
  (define (change-page/cell new-page)
    (web-cell-shadow table-ui-state-cell
                     (copy-struct table-ui-state (web-cell-ref table-ui-state-cell)
                                  [table-ui-state-current-page new-page])))
  (define (change-page-size/cell new-size)
    (define old-size (table-ui-state-objs-per-page (web-cell-ref table-ui-state-cell)))
    (define old-page (table-ui-state-current-page (web-cell-ref table-ui-state-cell)))
  ...
  (let/cc k
    ...
    (define (generate)  
      ...
      (define itus (web-cell-ref table-ui-state-cell))
      ...
      (define movement-div
        `(div ([class ,(css-class "movement")])
              ...
              (span ([class ,(css-class "movement-prev")])
                    ,(if (> current-page 0)
                         `(a ([href ,(url->string (embed/url (change-page (sub1 current-page))))])
                             larr nbsp "Previous")
                    ...
                    ,@(map (lambda (page)
                             `(span
                               ,(if (not (= page current-page))
                                    `(a ([href ,(url->string (embed/url (change-page page)))])
              ...
              (span ([class ,(css-class "movement-next")])
                    ,(if (< current-page last-page)
                         `(a ([href ,(url->string (embed/url (change-page (add1 current-page))))])
                             "Next" nbsp rarr)
                         `(span "Next" nbsp rarr)))))
                         ...
                         `(span 
                           ,(if (eq? n (table-ui-state-objs-per-page itus))
                                `(span ([class ,(css-class "count-current")])
                                       ,(number->string n))
                                `(a ([href ,(url->string (embed/url (change-page-size n)))]
                            ...
                            [(? table-column-obj-<= a-column)
                             ...
                             `(th ([class ,(css-column-class a-column)]
                                   [style ,(if ((list-ui-sorted-by? the-list-ui) a-column)
                                               ...
                                               "")]
                                   [scope "column"])
                                  (a ([class "k-url"] [href ,(url->string (embed/url proc))] [title ,title])

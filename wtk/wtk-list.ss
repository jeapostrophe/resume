...
(define-struct list-sort (id <= tag) #:prefab)
; filter : symbol (a -> boolean) b
(define-struct list-filter (id pred? tag) #:prefab)

(define-struct list-ui-state (sort-id reverse-sort? filters) #:prefab)
(define (with-list-ui:initial-state initial-sort)
  (make-web-cell (make-list-ui-state initial-sort #f (list))))

; list-ui
(define-struct list-ui (objects
                        map-filters clear-filters filter-by filtered-by?
                        map-sorts sort-by sorted-by? sort-reversed?) #:prefab)

; with-list-ui : (local-cell list-ui-state) (list (filter a b)) (list (sort a)) (-> (list a)) -> list-ui
(define (with-list-ui state-cell filters sorts gen-objects
                      #:key
                      [stable? #f]
                      [sort mergesort]
                      [and-filters? #t])
    ...
    (letrec ([gen-filter-test
                  ...
                  (map (lambda (a-filter-id) 
                         ...
                         (if list-filter
                             ...
                             (begin 
                               ...
                               (lambda (x) #t))))
                       (list-ui-state-filters (web-cell-ref state-cell))))                  
                (lambda (x)
                  (if (empty? preds)
                      #t
                      (if and-filters?
             ...
             [clear-filters
              (lambda (request)
                (web-cell-shadow
                 state-cell
                 (copy-struct list-ui-state (web-cell-ref state-cell)
                              [list-ui-state-filters (list)]))
                (redirect/get)
                (generate))]
             [filtered-by?
              (lambda (the-filter-id)
                (memq the-filter-id (list-ui-state-filters (web-cell-ref state-cell))))]
             [filter-by
              (lambda (the-filter-id)
                (lambda (request)
                  (define state (web-cell-ref state-cell))
                  (web-cell-shadow
                   state-cell
             ...
             [sorted-by?
              (lambda (the-sort-id)
                (eq? (list-ui-state-sort-id (web-cell-ref state-cell))
                     the-sort-id))]
             [sort-reversed?
              (lambda ()
                (list-ui-state-reverse-sort? (web-cell-ref state-cell)))]
             [sort-by
              (lambda (the-sort-id)
                (lambda (request)
                  (web-cell-shadow state-cell
                                   (copy-struct list-ui-state
                                                (web-cell-ref state-cell)
